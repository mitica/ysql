...
Postgres.prototype.visitExpression = function(node) {
  //console.log(JSON.stringify(node));
  var result = ['(', this.visit(node.left).join(''),' ' + node.operator];
  if(node.right){
    result = result.concat([' ',this.visit(node.right).join('')]);
  }
  result.push(')');
  if (node.relation)
    result.push(' ' + node.relation);
...
Postgres.prototype.visitParam = function(node) {
  ...
  this.params.push({name: node.name || 'p'+(this.params.length+1)});

  var result = [this._getParameterPlaceholder(this.params.length, node.value)];
  return result;
};

Postgres.prototype.visitValue = function(value) {
  if(value.value=== undefined || value.value === null) return [];
  var result = [this.quote(value.value.toString())];
  return result;
};
